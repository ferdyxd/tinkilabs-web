...
export function ShareSidebar({ url, title }: { url: string; title: string }) {
  ...
  return (
    <aside className="hidden lg:block">
      <div className="sticky top-24 flex flex-col items-center gap-1">
        <span className="text-[10px] font-medium text-tinki-dark/20 mb-1 tracking-widest">COMPARTIR</span>

        {/* WhatsApp */}
        <a href={`https://wa.me/?text=${encodedTitle}%20${encodedUrl}`} target="_blank" rel="noopener noreferrer" className={btn} style={{ color: '' }} aria-label="WhatsApp">
        ...
        <button onClick={handleCopy} className={`${btn} relative`} aria-label="Copiar enlace">
          <CopyIcon />
          {copied && (
            <span className="absolute -right-1 top-1/2 -translate-y-1/2 translate-x-full whitespace-nowrap rounded-full bg-tinki-dark px-2 py-0.5 text-[10px] font-medium text-white shadow-sm animate-fade-in">
              ¡Copiado!
            </span>
          )}
        </button>
        ...
        <button onClick={handleCopy} className={btn} aria-label="Copiar para Instagram">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" /><circle cx="12" cy="12" r="5" /><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" /></svg>
        </button>
